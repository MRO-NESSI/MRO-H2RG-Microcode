...
; Turn on the low voltages (+/- 6.5V, +/- 16.5V) and then delay awhile
	...
	JSR	<CLOCK
	MOVE	#>1,A			; Longer delay after reset RS 7/18/14
	JSR	<MILLISEC_DELAY		; Wait 1ms  RS 7/18/13
	JSR	<INIT_H2RG		; Initialize the H2RG array  
;	MOVE	#>1000,A		; TEMPORARY
;	JSR	<MILLISEC_DELAY		; TEMPORARY
	JMP	<FINISH
...
NEXT_FRAME
	...
	MOVE	A1,Y:<IBUFFER
	CMP	X0,A
	JLT	<RST_AR			; Test if the frame buffer is full
...
RST_AR	JSSET	#ST_RST_MODE,X:STATUS,RESET_ARRAY 	; Reset array if needed
...
; Initialize the internal registers to default settings. Modified 7/19/13 by LS and RS. 
; Sequence:
; 	OptionsReg
; 	NormalModeReg
; 	OutputBufReg
...
INIT_H2RG
	BSET	#ST_DIRTY,X:<STATUS
	MOVE	#(OPTIONS_REG+$00),A		; Serial on VCLK and FSYNCB
	JSR	<SER_COM		
	MOVE	#(NORMAL_MODE_REG+$80),A	; Standard clocking, global reset 
	JSR	<SER_COM
	MOVE	#(OUTPUT_BUF_REG+$02),A		; Slow bufferred readout mode, on B, no highohm
	JSR	<SER_COM			
	MOVE	#(HORI_DIR_REG+$00),A		; All left to right.
	JSR	<SER_COM
	MOVE	#(GAIN_REG+$00),A		; Disable fast readout features
	JSR	<SER_COM
	MOVE	#(WINDOW_MODE_REG+$80),A	; Windowing mode is standard clocking, global reset	
	JSR	<SER_COM
	MOVE	#(POWER_DOWN_REG+$00),A		; Default is $55, no power down
	JSR	<SER_COM
	MOVE	#(OUTPUT_MODE_REGISTER+$04),A	; 32-output mode, use pin #7 for window output
	JSR	<SER_COM
	MOVE	#(MISC_REG+$00),A		; Reset vertical and horizontal scanner
	JSR	<SER_COM
	MOVE	#(MISC_REG+$0C),A		; Enable vertical and horizontal scanner
	JSR	<SER_COM
	RTS
...
SET_RESET_MODE
	JCLR	#0,X:(R3)+,NO_RESET	; 1 to reset FPA in continuous 
